...
def recover_tasks_from_corrupt_file(path: Path) -> List[Task]:
    """Attempt to recover valid tasks from a corrupted JSON file."""
    ...
    for d in raw:
        try:
            tasks.append(_dict_to_task(d.copy()))
...
def load_tasks() -> List[Task]:
    ...
    try:
        ...
        tasks = recover_tasks_from_corrupt_file(DATA_FILE)
        return tasks

def save_tasks(tasks: List[Task]) -> None:
    try:
        DATA_FILE.parent.mkdir(parents=True, exist_ok=True)
        DATA_FILE.write_text(json.dumps([_task_to_dict(t) for t in tasks], indent=2), encoding='utf-8')
    except OSError as e:
